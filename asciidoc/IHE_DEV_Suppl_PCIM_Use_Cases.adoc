...
<p style="text-align: center">Revision 2.2 &mdash; Trial Implementation</p>
++++
...
This supplement is published on December 5, 2024 for trial implementation and may be available for testing at subsequent IHE Connectathons. The supplement may be amended based on the results
...
==== 7.4.2.1 Use Case #1: Identifiable person asserting a validated association

===== 7.4.2.1.1 Description

A Device-Patient Association Reporter asserts an identifiable person authored and validated a device-patient association to a Device-Patient Association Manager.

An authorized person at the point-of-care who can see the patient and devices has gathered and checked the unique identifying information for a patient and a device that is designated to originate observations on that patient.
Before being sent, the information is displayed to the person for validation.
Once validated, a message is originated by the Device-Patient Association Reporter with the following information:

|===
|Association Data|Optionality

|Patient identifier
|Required

|Device identifier and the begin time for the effective association
|Required

|Location identifier
|Optional

|Person identifier who authored and validated the association
|Required

|Time the association was validated
|Required

|Indicator that the association is final and does not require further validation
|Required
|===

Examples of reporters that this use case applies to are electronic medical records systems, specialized medical applications and medical devices or controllers with appropriate user interfaces and the ability to capture the identity of the person authorized to perform the association and validation.

===== 7.4.2.1.2 Process Flow

This use case can be driven by an authorized person responsible for scanning, selecting or entering and then validating the
beginning of an association between a device and a particular patient. This should be
based on first person awareness of the situation at the point-of-care. Automatic Identification and
Data Capture methods such as barcodes, picklists, or auto-complete with detailed confirmation should be used to assist the workflow and
...
The authorized person's identity has been collected and verified by the system.

===== 7.4.2.1.4 Main Flow:

Device-Patient Association Reporter originates a message with the specific information on the association and its time of beginning and validation by the authorized person.
...
After completion of this use case, an association record identifying the patient and the associated device and giving the start time of the association is received by and persisted by the Device-Patient Association Manager. 

TBD: Explain/reference how the manager resolves conflicts and what happens if it cannot resolve them. 

TBD: Explain/reference how the manager delivers the association to the consumer.

TBD: Explain/reference how the consumer receives the association.

==== 7.4.2.2 Use Case #2: Identifiable person reporting an association requiring validation

===== 7.4.2.2.1 Description

A Device-Patient Association Reporter asserts an identifiable person authored a device-patient association requiring validation to a Device-Patient Association Manager.

An authorized person at the point-of-care who can see the patient and devices has gathered and checked the unique identifying information for a patient and a device that is designated to originate observations on that patient.
The message is originated by the Device-Patient Association Reporter with the following information:
...
|Person identifier who authored the association
|Required

|Time the association was authored
|Required

|Indicator that the association requires further validation
|Required
|===

Examples of reporters that this use case applies to are electronic medical records systems, specialized medical applications and medical devices or controllers with appropriate user interfaces and the ability to capture the person doing the association but with a policy that requires an additional person using the Device-Patient Association Manager to validate the association.

===== 7.4.2.2.2 Process Flow

This use case can be driven by an authorized person responsible for scanning, selecting or entering the
beginning of an association between a device and a particular patient. This should be
based on first person awareness of the situation at the point-of-care. Automatic Identification and
Data Capture methods such as barcodes, picklists, or auto-complete with detailed confirmation should be used to assist the workflow and
increase data reliability to the maximum feasible extent. 
The key difference from Use Case #1 is that the intial association is not validated by the person performing the initial association, but rather requires a second person to validate the association.
...
Patient has been assigned unique identifier at registration which has been collected and verified at the point-of-care.
Device identity has been registered for use.
The identities of patient and device(s) have been collected and entered by an authorized person.
The authorized person's identity has been collected and verified by the system.

===== 7.4.2.2.4 Main Flow:

Device-Patient Association Reporter originates a message with the specific information on the association and its time of beginning by the authorized person.
When such an association message is received, the manager system is responsible for determining if any conflicting information is in the system and generating an appropriate error message to assist the responsible personnel in resolving the conflict. The manager system is also responsible for notifying the appropriate person to validate the association.

===== 7.4.2.2.5 Post-conditions:

After completion of this use case, an association record identifying the patient and the associated device and giving the start time of the association is received by and persisted by the Device-Patient Association Manager and the interaction requiring further validation is complete. 

TBD: Explain/reference how the manager resolves conflicts and what happens if it cannot resolve them. 

TBD: Explain how the manager HMI is used to validate the association.

TBD: Explain/reference how the manager delivers the association to the consumer.

TBD: Explain/reference how the consumer receives the association.

==== 7.4.2.3 Use Case #3: System, possibly with anonymous person input, asserting a validated association

===== 7.4.2.3.1 Description

A Device-Patient Association Reporter asserts a system authored a validated device-patient association to a Device-Patient Association Manager.

A system at the point-of-care that has access to the patient and devices has the ability to assert that a patient and device are associated without error. The system has access to the unique identifying information for a patient and a device that is designated to originate observations on that patient. Note, this system may be operated by an anonymous person or a system that does not have the ability to capture the identity of the person authorized to perform the association and validation.
...
|System identifier that authored and validated the association
|Required
...
Examples of reporters that this use case applies to are patient monitors, infusion pumps or other medical devices that have a picklist option but no means of user athentication with the device. Other examples are systems that have a means of identifying the patient and device automatically such as device aggregators or aother system that can detect the association without human intervention.

===== 7.4.2.3.2 Process Flow

This use case can be driven by system that is capable of detecting an association between a device and a particular patient without any chance of error.

These means of identification are specific to the technology and systems within the clinical environment in question, and standard procedures of risk analysis at the institution should be applied to assure that patient safety is adequately protected.

===== 7.4.2.3.3 Pre-conditions:

Patient is to be associated with a device for clinical observations.
Patient has been assigned unique identifier at registration which has been collected, detected and verified at the point-of-care.
Device is registered for use.
The identities of patient and device have been detected and verified by the system.
The system has been assigned a unique identifier.

===== 7.4.2.3.4 Main Flow:

Device-Patient Association Reporter originates a message with the specific information on the association and its time of beginning and validation by the system.
When such an association message is received, the manager system is responsible for determining if any conflicting information is in the system and generating an appropriate error message to assist the responsible personnel in resolving the conflict.
...
==== 7.4.2.4 Use Case #4: System, possibly with anonymous person input, reporting an association requiring validation

===== 7.4.2.4.1 Description

A Device-Patient Association Reporter asserts a system, possibly directed by a anonymous person, authored a device-patient association requiring validation to a Device-Patient Association Manager.

A system at the point-of-care that has access to the patient and devices has the ability to assert that a patient and device are associated, although validation by a person is required. The system has access to the unique identifying information for a patient and a device that is designated to originate observations on that patient. Note, this system may be operated by an anonymous person or a system that does not have the ability to capture the identity of the person authorized to perform the association.

 A message is originated by the Device-Patient Association Reporter with the following information:
...
|System identifier who authored and validated the association
|Required
...
Examples of reporters that this use case applies to are location based device aggregators that are installed at the bedside and sense devices and patients using proximity sensors, RTLS systems using RFID and other systems that can detect the association without human intervention. 

===== 7.4.2.4.2 Process Flow

This use case can be driven by a system responsible for deteting a medical device association with a patient at the point-of-care.
...
Patient has been assigned unique identifier at registration which has been collected and verified at the point-of-care.
Device is registered for use.
System is configured to detect the patient and device association.
...
When such an association message is received, the manager system is responsible for determining if any conflicting information is in the system and generating an appropriate error message to assist the responsible personnel in resolving the conflict.
The manager system is also responsible for notifying the appropriate person to validate the association.
...
==== 7.4.2.5 Use Case #5: Disassociating Device From Patient
...
|Device identifier and the end time for the association
|Required

|Location identifier
|Optional

|System or Person identifier that authored the disassociation message
|Required

|Time the disassociation message was authored
|Required

|Indicator that the disassociation is final and does not require further validation
|Required
|===

It should be noted that even though this may be a less salient event at the point-of-care, completeness and accuracy of disassociation is important to ensure an accurate record and proper association of observations with patients.
...
In some cases, the device itself is capable of determining that the association has been broken and can communicate this information to the Device-Patient Association Manager if it has implemented the Device-Patient Association Reporter role.
...
==== 7.4.2.6 Use Case #6 Filter Devices for a Patient

===== 7.4.2.6.1 Description
