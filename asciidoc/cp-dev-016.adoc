...
After PCIM connectathon testing in 2025, it was noted that OBX-1 for DEV-51 (Communicate Association State) and DEV-52 (Report Association State) transactions was set to be optional. This was inconsistent with PCD TF-2. During review, it was also noted that the description for OBX-4 needed to be clarified and made more consistent with expectations of containment with the TF. It was agreed on in the PCIM WG to change the optionality to required and to update the description for OBX-4.
...
| R
|
| Observation Sub-ID. 0.0.0.0 or 1.0.0.0 or 1.1.0.0? Used to convey the level in the containment hierarchy at which the patient is being associated. Note, this message originates from a reporter which, in the case of a RTLS system, may not have a MDS and VMD.
| 5
| CWE
| R
|
