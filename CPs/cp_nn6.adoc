...
|*3.17.4.1.2 Message Semantics*, _update last sentence on line 511, to represent association and disassociation and to suggest using a technical alert notify human user_:
...
[underline]#Proposed Text:#
[.text-left]
After these checks, the Manager logs the result and returns an appropriate positive or negative acknowledgement to the Reporter. The system design must assure that errors are indicated to the appropriate human user(s) in an effective and timely manner so that action can be taken. A technical alert may be raised using the ACM profile to satisfy this requirement, the details of this are out of scope for this document.
