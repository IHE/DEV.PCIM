...
|*3.17 Assert Device-Patient Association [PCD-17]*, _rename to_: *Assert Device-Patient Association [DEV-51] and all subsequent sections are named 3.51 instead of 3.52*
...
|*3.17.4.1.2 Message Semantics*, _update check #1 to not require manager to check user and system authorization:
...
Assertions and Report messages for DEV-51 and DEV-52 transactions, respectively, use the same structure, with the following differences:

  . A report must always have a OBX-11 status field of "Final"
